...
Una vez recopilada la información de tráfico, está listo para [profundizar](discovered-apps.md#deep-dive-into-discovered-apps) en el uso de aplicaciones en la nube en su organización. Cloud App Security aprovecha las ventajas de las funcionalidades de protección de redes de punto de conexión de Microsoft defender para bloquear el acceso de dispositivos de extremo a aplicaciones en la nube. Puede bloquear las aplicaciones si las [etiqueta como no **autorizadas**](governance-discovery.md#BKMK_SanctionApp) en el portal. En función de la evaluación de riesgos y uso integral de cada aplicación no autorizada, los dominios de la aplicación se usan para crear [indicadores de dominio](/windows/security/threat-protection/microsoft-defender-atp/manage-indicators#create-indicators-for-ips-and-urlsdomains-preview) en Microsoft defender para el portal de puntos de conexión. Antivirus de Microsoft defender, que se ejecuta en dispositivos de punto de conexión, usa los indicadores de dominio para bloquear el acceso a estas aplicaciones.
...
Por su cuenta, Cloud App Security recopila registros de los puntos de conexión [usando registros cargados](create-snapshot-cloud-discovery-reports.md) o [configurando la carga automática de registros](discovery-docker.md). La integración nativa le permite aprovechar los registros que Microsoft defender para el agente del punto de conexión crea cuando se ejecuta en Windows y supervisa las transacciones de red. Use esta información para la detección de Shadow IT en los equipos Windows de la red.

Para que pueda realizar Cloud Discovery en otras plataformas, es mejor usar tanto el compilador de [registros](discovery-docker.md)de Cloud App Security, junto con Microsoft defender para que la integración de puntos de conexión supervise sus máquinas con Windows 10.

[Vea nuestros vídeos](#related-videos) en los que se muestran las ventajas de usar Microsoft defender para el punto de conexión con Cloud App Security.
...
Para habilitar Microsoft defender para la integración de puntos de conexión con Cloud App Security:

1. En el panel de navegación de Microsoft defender para el portal de punto de conexión, seleccione **preferencias configuración** .
...
![Microsoft defender para la configuración del punto de conexión](media/wdatp-settings.png)

## <a name="investigate-machines-in-cloud-app-security"></a>Investigar las máquinas de Cloud App Security

Después de integrar Microsoft defender para el punto de conexión con Cloud App Security, puede investigar los datos de la máquina detectada en el panel de Cloud Discovery.
...
5. Puede explorar en profundidad cada equipo que se muestra y usar las pestañas para ver los datos de la investigación. Busque correlaciones entre los equipos, los usuarios, las direcciones IP y las aplicaciones que estuvieron implicados en incidentes:
    ...
    - **Información general**
        - **Nivel de riesgo del equipo** : muestra el riesgo de que el perfil del equipo sea relativo a otras máquinas de la organización, tal y como indica la gravedad (alta, media, baja e informativa). Cloud App Security usa perfiles de equipo de Microsoft defender para el punto de conexión para cada equipo basado en análisis avanzado. La actividad que es anómala en la línea de base de un equipo se evalúa y determina el nivel de riesgo de la máquina. Use el nivel de riesgo del equipo para determinar qué máquinas se deben investigar en primer lugar.
...
## <a name="investigate-app-usage-in-microsoft-defender-for-endpoint-with-advanced-hunting"></a>Investigar el uso de aplicaciones en Microsoft defender para el punto de conexión con la búsqueda avanzada

Siga estos pasos para obtener una visibilidad más granular sobre los eventos de red relacionados con la aplicación en Microsoft defender para el punto de conexión:
...
1. En Cloud App Security, en el engranaje de configuración, seleccione **configuración** , en **Cloud Discovery** seleccione **Microsoft defender para punto de conexión** y, a continuación, seleccione **bloquear aplicaciones no autorizadas** .
    ...
    ![Captura de pantalla que muestra cómo habilitar el bloqueo con Microsoft defender para el punto de conexión](media/defender-atp-integration.png)
...
1. En Microsoft defender Security Center, vaya a **configuración**  >  **características avanzadas** y, a continuación, seleccione **indicadores de red personalizados** . Para obtener información acerca de los indicadores de red, consulte [crear indicadores para direcciones IP y direcciones URL y dominios](/windows/security/threat-protection/microsoft-defender-atp/manage-indicators#create-indicators-for-ips-and-urlsdomains-preview).
    ...
    ![Captura de pantalla que muestra cómo habilitar los indicadores de red personalizados en Microsoft defender para el punto de conexión](media/defender-atp-custom-network-indicators.png)
...
A veces, no se bloquea el acceso a una aplicación no autorizada, ya sea porque el dispositivo de punto de conexión no está configurado correctamente o si la Directiva de cumplimiento no se ha propagado todavía al punto de conexión. En esta instancia, Microsoft defender para los administradores de puntos de conexión recibirá una alerta en Microsoft defender Security Center de que no se bloqueó la aplicación no autorizada.

![Captura de pantalla que muestra la alerta de aplicación no autorizada de Microsoft defender para puntos de conexión](media/defender-atp-unsanctioned-app-alert.png)
...
> [!div class="nextstepaction"]
> [Detección y bloqueo de la sombra con Microsoft defender para el punto de conexión](https://www.youtube.com/watch?v=MsHkTOoqSQo)
