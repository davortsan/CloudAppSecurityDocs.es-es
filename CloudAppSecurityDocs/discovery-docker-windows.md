...
2. Cree un origen de datos coincidente para cada firewall o servidor proxy desde el que quiera cargar registros.
    ...
    f. Repita este proceso para cada servidor proxy y firewall cuyos registros se puedan usar para detectar tráfico en la red. Se recomienda configurar un origen de datos dedicado por dispositivo de red para que pueda: supervisar el estado de cada dispositivo por separado, con fines de investigación.
        - Explorar Shadow IT Discovery de cada dispositivo, si cada uno de ellos lo utiliza un segmento de usuarios distinto.
...
2. Ejecute el siguiente comando para descargar el archivo de script de PowerShell del instalador de Docker de Windows: `Invoke-WebRequest https://adaprodconsole.blob.core.windows.net/public-files/LogCollectorInstaller.ps1 -OutFile (Join-Path $Env:Temp LogCollectorInstaller.ps1)`

    Para comprobar que el instalador está firmado por Microsoft, consulte [Validate Installer Signature](#validate-signature) .

3. Para habilitar la ejecución de scripts de PowerShell, ejecute `Set-ExecutionPolicy RemoteSigned`

4. Ejecutar: `& (Join-Path $Env:Temp LogCollectorInstaller.ps1)` instala el cliente de Docker en la máquina. Durante la instalación del contenedor de recopilador de registros, la máquina se reiniciará dos veces y tendrá que volver a iniciar sesión. **Asegúrese de que el cliente de Docker está configurado para usar contenedores de Linux.**

5. Después de cada reinicio, abra un terminal de PowerShell como administrador en la máquina, vuelva a ejecutar: `& (Join-Path $Env:Temp LogCollectorInstaller.ps1)`
